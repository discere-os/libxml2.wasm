...
def main():
    ...
    for name in os.listdir(wasm_dir):
        if name.startswith('liblibxml2-side') and name.endswith('.wasm'):
            src = os.path.join(wasm_dir, name)
            dst = os.path.join(wasm_dir, 'libxml2-side.wasm')
            if os.path.exists(dst):
                os.remove(dst)
            shutil.move(src, dst)
        if name.startswith('libxml2-side') and name.endswith('.wasm'):
            src = os.path.join(wasm_dir, name)
            dst = os.path.join(wasm_dir, 'libxml2-side.wasm')
            if os.path.exists(dst):
                os.remove(dst)
